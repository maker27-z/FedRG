...
class BaseHeadSplit(nn.Module):
    def __init__(self, args, hete_model, id):
        super().__init__()

        self.hete_model = hete_model
        self.id = id
        self.base = eval(self.hete_model[self.id % len(self.hete_model)]).to(args.device)
        ...
        if hasattr(args, 'heads'):
            ...
            self.head = nn.Linear(args.feature_dim, args.num_classes).to(args.device)
...
#     def __init__(self, input_dim=1*28*28, dim=512, num_classes=10):
...
class DNN(nn.Module):
    def __init__(self, input_dim=1*28*28, mid_dim=512, num_classes=10):
...
class FedAvgCNN(nn.Module):
    def __init__(self, in_features=1, num_classes=10, dim=1024, feature_dim=512):
        super().__init__()
        self.conv1 = nn.Sequential(
